...
program_code = """functions {
    ...
    vector[] projected_position_2d(int T, vector t, real a, real P, real eccentricity, real cos_inclination, real Omega_, real omega_, real t_peri) {
        ...
        */
        ...
        // something about campbell and reference frames means we need to switch x,y here?!?!
        return {xyz[2], xyz[1]};
    }
...
data {
    ...
    vector[T] ast_noise;
    matrix[T, T] ACAT;
    //vector[T] gaia_ra_col;
    //vector[T] gaia_dec_col;
    real al_err;
    real parallax;
    real ra;
    real dec;
...
parameters {
    // TODO: reparameterise this as M1, M2, P, eccentricity
    real<lower=0.2, upper=3> m1; // solar masses
    real<lower=0.2, upper=m1> m2; // solar masses
    // Can only infer (P, e) if we are using RUWE
    real<lower=0.1, upper=40> P;
    real<lower=0.0, upper=0.8> e;
    // Use `initial_phase` so that we can bound it between -pi and pi without having multi-modality
    // Transform to `t_peri` in transformed parameters block
...
model {
    ...
    //P ~ normal(3,1);
    //e ~ normal(0.5,0.1);
    //pmra_real ~ normal(-10,0.5);
    //t_peri ~ normal(0.15,0.05);
    //pmdec_real ~ normal(-4,1);
    //plx_real ~ normal(parallax,sqrt(covar_matrix[1,1]));
    //pmra_real ~ normal(pmra,sqrt(covar_matrix[2,2]));
    //pmdec_real ~ normal(pmdec,sqrt(covar_matrix[3,3]));
    {
        real a0 = a*(q/(1+q)-l/(1+l)); // l = 0 for a dark companion
        real a1 = a*(q/(1+q));
        //vector[T] rv_all = radial_velocity(T, t, a*dist/1000, q, l, P, eccentricity,cos_inclination, Omega, omega, t_peri);
        //real rv_semiamp = (max(rv_all)-min(rv_all))/2;
        ...
        //vector[5] obs_vals = gaia_params(T, sin_scan_angle, cos_scan_angle, total_diffs[1], total_diffs[2], inv_gaia);
        ...
        real mag_sim = mag_from_mass(m1,m2,plx_real);
...
corr_names = ['ra_dec','ra_parallax','ra_pmra','ra_pmdec','dec_parallax','dec_pmra','dec_pmdec','parallax_pmra','parallax_pmdec','pmra_pmdec']
all_ruwes = []
all_plxs = []
all_mags = []
all_ids = []
with open(inFile, newline='') as csvfile:
	...
	for row in reader:
		if row['ruwe']=='':
			continue
		all_ruwes.append(float(row['ruwe']))
		all_plxs.append(float(row['parallax']))
		all_mags.append(float(row['phot_g_mean_mag']))
		all_ids.append(row['designation'][9:len(row['designation'])])
all_ruwes = np.array(all_ruwes)
all_plxs = np.array(all_plxs)
all_mags = np.array(all_mags)
young_ids = []
use_ruwes = []
use_ages = []
n_star = len(all_ruwes)
star_ii = 0
with open(inFile, newline='') as csvfile:
	reader = csv.DictReader(csvfile)
	for row in reader:
		use_id = row['designation'][9:len(row['designation'])]
		gaia_params = []
		gaia_errs = []
		gaia_corr = []
		counter = 0
		for ii in range(len(param_names)):
			if row[param_names[ii]]=='':
				counter+=1
		if counter>0:
			continue
		for ii in range(len(param_names)):
			gaia_params.append(float(row[param_names[ii]]))
			gaia_errs.append(float(row[param_names[ii]+'_error']))
		for ii in range(len(corr_names)):
			gaia_corr.append(float(row[corr_names[ii]+'_corr']))
		...
		pickle.dump((t,scan_angle,plx_factor),open('obs_'+system_name+'.pkl','wb'))
		#continue
		#(t,scan_angle,plx_factor) = pickle.load(open('obs_'+use_id+'.pkl','rb'))
		nSamps = 4000
		nChains = 1
		A = design_matrix(t-2016, scan_angle, plx_factor)
		C = np.linalg.solve(A.T @ A, np.eye(5))
		ACAT = A @ C @ A.T
		gaia_mat = gaia_matrix(t-2016,ra,dec)
		gaia_mat_AL = gaia_matrix_AL(t-2016,scan_angle,ra,dec)
		inv_gaia = gaia_inverse_AL(t-2016,scan_angle,ra,dec)
		abs_mag = g_mag-5*np.log10(100./plx_obs)
		m1_mean = 1#-0.145*abs_mag+1.64
		ast_error = 0.1#float(astromet.sigma_ast(g_mag))
		all_samples = {}
		ast_noise = ast_error*np.random.randn(len(t))#np.random.normal(0,ast_error/2,len(t))
		data = {
			"ra": ra, 
			"dec": dec, 
			"pmra": pmra_obs, 
		...
		pickle.dump(samples,open('samples_'+use_id+'.pkl','wb'))
